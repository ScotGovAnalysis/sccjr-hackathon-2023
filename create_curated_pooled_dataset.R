...
scjs_combined_pool <- left_join(scjs_combined, pooling_info, by = c("survey_year" = "year"))

summary_info <- scjs_combined_pool %>% 
  select(survey_year, wgtgindiv) %>% 
  group_by(survey_year) %>% 
  summarise(sum_weight = sum(wgtgindiv))

scjs_combined_pool <- left_join(scjs_combined_pool, summary_info, by = c("survey_year"))

scjs_combined_pool <- scjs_combined_pool %>%
